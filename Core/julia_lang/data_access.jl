...
function iter_param(cursor::SQLite.Cursor, param_name::String)
...
function get_param(cursor::SQLite.Cursor, param_name::String, indices...)
    if haskey(Parser.param_index_dict, param_name)
        index_dict = Parser.param_index_dict[param_name]
        query = ""
        x = nothing
...
function get_set(cursor::SQLite.Cursor, set_name::String)
    if set_name == "time"
...
function get_discount_factor(cursor::SQLite.Cursor, y::Int)
    y_0 = _get_set(cutsor, "year")[1]
    return (1 + get_param(cursor, "discount_rate")) ^ (y_0 - y)
end
